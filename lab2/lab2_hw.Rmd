...
> Vector is a basic data structure in R. It contains elements of the same type. The data types can be logical, numeric, integer, character, complex.
...
```{r}
Bluebell <- springs_matrix[1,]
Bluebell
mean(Bluebell)
Opal <- springs_matrix[2,]
Opal
mean(Opal)
```
```{r}
all_mean <- c(Bluebell,Opal)
all_mean
```
